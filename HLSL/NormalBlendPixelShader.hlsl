...
float4 main(PSInput input) : SV_TARGET {
    ...
    if (alphaWdightIndex > 0.001f) {
        float2 wdightUV = float2(saturate(alphaWdightIndex * 0.13f), 0.0f);
        float alphaWdight = saturate(wdightTex.Sample(samp, wdightUV).a);
        outputColor = color * alphaWdight +  (1.0f - alphaWdight) * backgroundColor;
    } else {
