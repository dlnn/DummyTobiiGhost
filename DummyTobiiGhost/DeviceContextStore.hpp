...
#define SAVE_SHADER_STATE(shaderStage, shaderName) \
    void shaderStage##SetShader(ID3D11##shaderName* shader, ID3D11ClassInstance* const* classInstances, UINT numClassInstances) \
    { \
        ...
        _pDeviceContext->shaderStage##SetShader(shader, classInstances, numClassInstances); \
    } \
    void shaderStage##SetConstantBuffers(UINT startSlot, UINT numBuffers, ID3D11Buffer* const* constantBuffers) \
    { \
        MARK_DIRTY(shaderStage##SetConstantBuffers, shaderStage##GetConstantBuffers, startSlot, numBuffers, _originalState.shaderName##ConstantBuffers); \
        _pDeviceContext->shaderStage##SetConstantBuffers(startSlot, numBuffers, constantBuffers); \
    } \
    void shaderStage##SetShaderResources(UINT startSlot, UINT numViews, ID3D11ShaderResourceView* const* shaderResourceViews) \
    { \
        MARK_DIRTY(shaderStage##SetShaderResources, shaderStage##GetShaderResources, startSlot, numViews, _originalState.shaderName##ShaderResourceViews); \
        _pDeviceContext->shaderStage##SetShaderResources(startSlot, numViews, shaderResourceViews); \
    } \
    void shaderStage##SetSamplers(UINT startSlot, UINT numSamplers, ID3D11SamplerState* const* samplers) \
    { \
        MARK_DIRTY(shaderStage##SetSamplers, shaderStage##GetSamplers, startSlot, numSamplers, _originalState.shaderName##Samplers); \
        _pDeviceContext->shaderStage##SetSamplers(startSlot, numSamplers, samplers); \
    }
...
public:
	...
	ID3D11DeviceContext* GetRawContext()
	{
...
#pragma region InputAssembler
	...
	{
		MARK_DIRTY(IASetVertexBuffers, IAGetVertexBuffers, startSlot, numBuffers, &_originalState.VertexBuffer, &_originalState.VertexBufferStride, &_originalState.VertexBufferOffset);
...
#pragma region Rasterizer
	void SOSetTargets(UINT numBuffers, ID3D11Buffer* const* buffers, const UINT* offsets)
	{
	...
	void RSSetViewports(UINT numViewports, const D3D11_VIEWPORT* viewports)
	{
		...
		_pDeviceContext->RSSetViewports(numViewports, viewports);
	}

	void RSSetScissorRects(UINT numRects, const D3D11_RECT* rects)
	{
		MARK_DIRTY(RSSetScissorRects, RSGetScissorRects, &_originalState.NumScissorRects, _originalState.ScissorRects);
		_pDeviceContext->RSSetScissorRects(numRects, rects);
	}
#pragma endregion

#pragma region OutputMerger
	void OMSetRenderTargets(UINT numViews, ID3D11RenderTargetView* const* renderTargetViews, ID3D11DepthStencilView* depthStencilView)
	{
		...
		_pDeviceContext->OMSetRenderTargets(numViews, renderTargetViews, depthStencilView);
	}

	void OMSetRenderTargetsAndUnorderedAccessViews(UINT numRTVs, ID3D11RenderTargetView* const* renderTargetViews, ID3D11DepthStencilView* depthStencilView, UINT uavStartSlot, UINT numUAVs, ID3D11UnorderedAccessView* const* uavs, const UINT* uavInitialCounts)
	{
		MARK_DIRTY(OMSetRenderTargetsAndUnorderedAccessViews, OMGetRenderTargetsAndUnorderedAccessViews, numRTVs, _originalState.RenderTargetViews, &_originalState.DepthStencilView, uavStartSlot, numUAVs, _originalState.UnorderedAccessViews);
		_pDeviceContext->OMSetRenderTargetsAndUnorderedAccessViews(numRTVs, renderTargetViews, depthStencilView, uavStartSlot, numUAVs, uavs, uavInitialCounts);
	}
	...
	void OMSetBlendState(ID3D11BlendState* blendState, const FLOAT blendFactor[4], UINT sampleMask)
	{
		...
		_pDeviceContext->OMSetBlendState(blendState, blendFactor, sampleMask);
	}
	...
	void OMSetDepthStencilState(ID3D11DepthStencilState* depthStencilState, UINT stencilRef)
	{
		...
		_pDeviceContext->OMSetDepthStencilState(depthStencilState, stencilRef);
	}
#pragma endregion

#pragma region Other
	void SetPredication(ID3D11Predicate* predicate, BOOL predicateValue)
	{
		MARK_DIRTY(SetPredication, GetPredication, &_originalState.Predicate, &_originalState.PredicateValue);
		_pDeviceContext->SetPredication(predicate, predicateValue);
	}
#pragma endregion
	...
	void Restore()
	{
		...
		RESTORE_NORMAL_STATE(IASetVertexBuffers, 0, 1, &_originalState.VertexBuffer, &_originalState.VertexBufferStride, &_originalState.VertexBufferOffset)
